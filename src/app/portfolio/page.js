...
export default async function Portfolio({ params }) {
  ...
  // 3. 링크 클릭 시
  const safePage = Math.min(page, pageCount); // page 1이면 1
  const from = (safePage - 1) * PAGE_SIZE; // page 1이면 from 0
  const to = from + PAGE_SIZE - 1; // page 1이면 to 5
  console.log(from, to);

  // 4. 페이지 그룹 계산
  const pageGP = Math.ceil(safePage / PAGEGP_SIZE); // safePage 1이면 1
  const pageGPCount = Math.ceil(pageCount / PAGEGP_SIZE); // 8/5 1.6 => 2

  const groupStart = (pageGP - 1) * PAGEGP_SIZE + 1; // page 1~5면 1
  const groupEnd = Math.min(groupStart + (PAGEGP_SIZE - 1), pageCount); // pageStart 1이면 5
